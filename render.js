// render.js
    ...
    export class LatticeView {
      constructor(canvas) {
        ...
        this.scale = 80;        // pixels per unit (pentagon edge ~= 1)
        this.offset = { x: 0, y: 0 };
       ...
       this.ca = null;
        this.options = {
          // coloring
          colorMode: "sheet",      // "sheet" | "orient" | "depth" | "flat"
          palette: "hsl",          // "hsl" | "warm" | "cool" | "mono"
          saturation: 60,
          lightness: 56,
          // fill/stroke
          fillTiles: true,
          ...
          glowStrength: 14,
         // CA overlay
        caOverlay: true,         // when true, color by CA state instead of sheet/etc.
         caDeadAlphaSel: 0.20,    // alpha for dead cells on selected sheet
         caDeadAlphaOther: 0.06,  // alpha for dead cells on other sheets
         caLiveBoost: 1.0,        // brightness boost multiplier for live cells
        };
        this._dpr = 1;
        this._fitDPI();
        // Use ResizeObserver so we catch all layout changes, not just window resize.
      ...
      setLattice(l) {
        this.lattice = l;
        this.selectedIdx = 0;
        // Make sure size is current before fitting.
        this._fitDPI();
        this.fit();
        this.draw();
      }
     setCA(ca) {
      ...
      fit() {
        ...
        // world point (cx, cy) should map to canvas center
      ...
      // Convert raw pointer event coordinates (clientX/Y) to canvas-local
      // CSS pixels, correctly handling any CSS scaling.
      eventToCanvas(ev) {
      ...
      pickTile(sx, sy) {
        ...
        // Prefer hits on the currently-selected sheet (topmost layer).
      ...
      zoom(factor, cx, cy) {
        // zoom around (cx, cy) in screen coords
      ...
      draw() {
        ...
        // Group tiles by sheet for ordered translucent rendering.
      ...
      _drawTile(t, alpha, isSelSheet) {
        ...
        ctx.save();
       // When CA overlay is on, dead cells fade so live structure pops.
       let useAlpha = alpha;
       let isLive = false;
       if (opts.caOverlay && this.ca) {
         ...
         if (!isLive) {
          ...
          // Boost live-cell visibility so non-selected sheets still pop.
          useAlpha = isSelSheet ? Math.max(alpha, 0.95)
                                : Math.max(alpha, 0.6);
         }
       }
      ...
      _tileColor(t, isSelSheet) {
        const opts = this.options;
       // CA overlay mode: color by cell state, with a calmer palette so
       // sheet structure stays subtly visible through transparency.
       if (opts.caOverlay && this.ca) {
         const v = this.ca.state[t.index] | 0;
         const ns = this.ca.numStates;
         if (v === 0) {
           // dead: use a desaturated sheet hint
           return paletteColor(t.sheet,
             Math.max(this.lattice.groupOrder, 1),
             "mono", 20, 30, isSelSheet);
         }
         // live: vivid palette indexed by state value
    ...
    function paletteColor(val, modulus, palette, sat, light, vivid) {
      ...
      if (palette === "warm") {
        const h = 0 + frac * 80; // red -> yellow
        return `hsl(${h}, ${s}%, ${l}%)`;
      }
      if (palette === "cool") {
        const h = 180 + frac * 100; // cyan -> violet
        return `hsl(${h}, ${s}%, ${l}%)`;
      }
      if (palette === "mono") {
        const ll = 20 + frac * 60;
   ...
   // CA live-cell coloring. For binary CA (numStates=2) we use a single
   // warm color; for multistate, we spread states around the palette.
   function caStateColor(val, numStates, palette, sat, light, vivid) {
     ...
     // multistate: spread evenly around palette ring, skipping state 0.

...
export function walkStatistics(
  cluster,
  { walks = 200, steps = 20, rng, root = cluster.originId, msdWindow = [2, 6], p0Window = [2, 12] }
) {
...
export function intraSheetStatistics(
  cluster,
  { walks = 100, steps = 20, rng, msdWindow = [2, 6] }
) {
...
export function commuteScaling(cluster, { rng, samples = 40, maxSteps = 400, maxDistance = 4 }) {
  ...
  for (let d = 1; d <= maxDistance; d++) {
    ...
    for (let i = 0; i < dist.length && found < 2; i++)
      if (dist[i] === d) {
        targets.push({ id: i, d, sheet: cluster.cells[i].sheet });
        found++;
      }
  }
  const data = [];

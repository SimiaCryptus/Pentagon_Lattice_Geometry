...
import {
  bfsVolumes, estimateDeff, countTriangles, meanClustering,
  girthAtNode, sheetTransitionMatrix, perSheetStats,
} from '../graph/analysis.js';
import { laplacianSpectrum, runKpm } from '../graph/spectral.js';
import {
  walkStatistics, intraSheetStatistics, commuteScaling, alexanderOrbach,
...
import {
  distanceCatalog, fibonacciSpacing, multiplicityAnalysis, galoisPairs,
  pythagoreanTriples, distinctDistanceScaling, distanceWeb,
  directionAnalysis, compareWebToAdjacency,
...
export const latticeParamSchema = [
  ...
  { key: 'preset', label: 'preset', type: 'select', default: 'small', options: ['tiny', 'small', 'medium', 'large', 'huge'] },
...
export function runLatticeExperiment(opts = {}, logger = new Logger()) {
  ...
  logger.log('interior angle =', geom.interiorAngleDeg, 'deg   k_flat =', geom.kFlat,
    '  deficit =', geom.deficitDeg, 'deg');
  logger.log('vertex-loop length =', geom.loopClosure,
  ...
  logger.log('degree min/mean/max =', stats.degreeMin, '/', stats.degreeMean, '/', stats.degreeMax,
    ` (interior target ${n})`);
  logger.log('sheets =', stats.sheets, ' vortex edges =', stats.vortexEdges,
    ' fraction =', stats.vortexFraction);
  ...
  const walk = walkStatistics(cluster, {
    walks: cfg.walks, steps: cfg.steps, rng,
  ...
  const intra = intraSheetStatistics(cluster, {
    ...
    steps: Math.min(cfg.steps, 100), rng,
    msdWindow: [2, Math.max(4, Math.min(cfg.steps, depth))],
  });
  ...
  logger.log('d_w intra-sheet =', intra.dWIntra,
    '  mean sheet crossings/walk =', walk.meanSheetCrossings);
  ...
  logger.log('Z_2 non-trivial =', holo.perGroup[0].nontrivial, '/', holo.cycleCount,
    '  residues =', holo.perGroup[0].residues);
  logger.log('Z_2 parity mismatches =', holo.parityMismatches, '(should be 0 for tauMode=z2)');
  logger.log('vertex loop length =', holo.vertexLoopLength,
    ' holonomy =', holo.vertexLoopHolonomy,
  ...
  logger.rule();
  logger.log('triangles =', triangles, ' mean clustering =', clustering.mean,
    ' girth(origin) =', Number.isFinite(girth) ? girth : '> 8');
  for (const s of perSheet)
    logger.log('  sheet', s.sheet, ': nodes =', s.nodes, ' intra-edges =', s.intraEdges,
      ' meanIntraDeg =', s.meanIntraDegree);

  /* --- Section 18: KPM --- */
  let kpm = null;
  if (runKPM) {
    kpm = runKpm(cluster, {
      moments: cfg.kpmMoments, samples: cfg.kpmSamples, rng,
      degreeMean: stats.degreeMean, degreeMax: stats.degreeMax,
    });
    logger.rule();
    logger.log('KPM mu_0 =', kpm.rescale.mu0, '(should be ~1)');
    logger.log('KPM d_spec (low-lambda tail) =', kpm.dSpecTail,
      '  d_spec (integrated DOS) =', kpm.dSpecCdf);
  }
  ...
  if (runCA) {
    ...
    logger.rule();
    logger.log(`CA default B${b}/S${b - 1}${b}: fate =`, defaultRun.fate,
      ' final pop =', defaultRun.finalPop, ' max pop =', defaultRun.maxPop);
    logger.log('rule panel:', JSON.stringify(survey.classify));
    if (survey.best) logger.log('most active rule:', survey.best.label, ' max pop =', survey.best.maxPop);
    logger.log('glider hunt: oscillators =', gliders.oscillators, ' gliders =', gliders.gliders,
      ' trials =', gliders.trials.length);
  }
  ...
  if (runErdos) {
    ...
    const directions = directionAnalysis(cluster, cat.classes, { count: opts.directionClasses ?? 6 });
    let webs = [];
    if (runWebs) {
    ...
    erdos = { catalog: { count: cat.count, pairs: cat.pairs, capped: cat.capped, classes: cat.classes.slice(0, 40).map(slimClass) }, spacing, mult, galoisPairs: gp, pythagoreanTriples: pyth, scaling, directions, webs };
    logger.rule();
    logger.log('distinct squared distances =', cat.count,
      cat.capped ? `(pair scan capped at ${cfg.maxPairs})` : '');
    logger.log('mean spacing ratio =', spacing.mean, ' phi^2 =', spacing.phiSquared,
      ' deviation =', spacing.deviation);
    logger.log('high-multiplicity rings =', mult.highMultiplicity.length,
      ' Galois pairs =', gp.length, ' Pythagorean triples =', pyth.length);
    if (scaling) logger.log('D(P_n)/log n =', scaling.ratioToLogN,
      ' (O(1) expected; Z^2 bound ~', scaling.integerLatticeBound, ')');
    logger.log('pinwheel classes =', directions.pinwheelCount, '/', directions.classes.length,
      '  direction growth slope =', directions.growthSlope);
    for (const w of webs)
      logger.log(`  W_${w.index}: edges=${w.edges} deg=[${w.degreeMin},${w.degreeMax}]`,
  ...
  const commute = commuteScaling(cluster, {
    rng, samples: Math.max(20, Math.min(200, Math.floor(cfg.walks / 100))),
    maxSteps: Math.max(100, cfg.steps), maxDistance: Math.min(depth, 4),
  });

  const elapsedMs = Date.now() - t0;
  logger.rule('=');
  logger.log('summary: N =', stats.N, ' d_eff =', dEffInterior.slope,
    ' d_w =', walk.dW, ' d_spec(P0) =', walk.dSpecP0, ' d_spec(AO) =', dSpecAO);
  ...
  return {
    ...
    spectrum: spectrum.skipped ? spectrum : {
      spectralGap: spectrum.spectralGap, lambdaMax: spectrum.lambdaMax, dos: spectrum.dos,
    },
    kpm,
    walk: { msd: walk.msd, p0: walk.p0, meanSheetCrossings: walk.meanSheetCrossings },
    holonomy: holo,
    structure: { triangles, clustering, girth: Number.isFinite(girth) ? girth : null, sheetMatrix, perSheet },
    automaton: ca,
    erdos,
    commute,
    elapsedMs,
...
const slimClass = (c) => ({
  index: c.index, value: c.value, radius: c.radius,
  ringSize: c.ringSize, pairCount: c.pairCount,
  q5: c.q5 ?? null, zphi: c.zphi ?? null, norm: c.norm ?? null, fibIndex: c.fibIndex ?? 0,
});
...
function buildTables({ stats, vols, dEffInterior, walk, holo, erdos, ca }) {
  ...
  if (erdos) {
    tables.push({
      ...
      rows: erdos.catalog.classes.map((c) => [c.index, c.value, c.radius, c.ringSize, c.pairCount, c.norm ?? '', c.fibIndex || '']),
    });
    tables.push({
      title: 'Direction analysis (pinwheel)',
      columns: ['k', 'd^2', '#directions', 'maxGap', 'pinwheel'],
      rows: erdos.directions.classes.map((d) => [d.index, d.value, d.directions, d.maxGap, d.pinwheel]),
    });
  }
  if (ca) {
    tables.push({
      ...
      rows: ca.survey.results.map((r) => [r.label, r.fate, r.period, r.transient, r.maxPop, r.finalPop]),
    });
  }
  tables.push({

...
import { PHI, PSI, fib, zphiNorm, q5Float, q5Norm, q5ToZphi } from '../math/numberTheory.js';
...
export const erdosParamSchema = [
  ...
  {
    key: 'crossSheetFraction',
    label: 'cross-sheet fraction',
    type: 'number',
    default: 1,
    step: 0.05,
  },
...
export function countNormRepresentations(target, searchMax) {
  ...
  for (let x = -searchMax; x <= searchMax; x++)
    for (let y = -searchMax; y <= searchMax; y++)
      if (zphiNorm([x, y]) === target) {
        count++;
        if (reps.length < 12) reps.push([x, y]);
      }
...
export function runErdosExperiment(opts = {}, logger = new Logger()) {
  ...
  logger.rule();
  logger.log(
    'Fibonacci ring spacing: mean ratio =',
    meanRatio,
    ' phi^2 =',
    PHI * PHI,
    ' deviation =',
    Math.abs(meanRatio - PHI * PHI)
  );

  /* Section 5: norm representation counts. */
  const normCounts = [1, -1, 4, -4, 5, 9, -9, 11, 16, -16, 19, 20, 25, 29, 31, 36].map((t) =>
    countNormRepresentations(t, searchMax)
  );
  logger.rule();
  ...
  const catalog = RING_CATALOG.map((r, i, arr) => {
    ...
    return {
      ...
      zphi: q5ToZphi(r.q5),
      norm: q5Norm(r.q5),
      ratio: prevVal ? value / prevVal : 0,
    };
  });
  logger.rule();
  logger.table(
    ['k', 'delta', '(r,s)', 'decimal', '|R_k|', 'ratio', 'notes'],
  ...
  const scaling = logNPoints.map((n) => {
    ...
    return {
      ...
      D,
      ratio: D / Math.log(n),
      integerLattice: n / Math.sqrt(Math.log(n)),
    };
  });
  const meanRatioD = scaling.reduce((s, r) => s + r.ratio, 0) / scaling.length;
  logger.rule();
  logger.table(
    ['n', 'log n', 'D(n)', 'D/log n', 'Z^2 bound'],
    scaling.map((r) => [r.n, r.logN, r.D, r.ratio, r.integerLattice]),
    [8, 10, 8, 12, 14]
  );
  logger.log('mean D(n)/log n =', meanRatioD, ' expected ~', 2 / (dEff * Math.log(PHI * PHI)));
  ...
  logger.rule();
  logger.log(
    'cross-sheet family sizes (fraction =',
    crossFrac,
  ...
  for (let k = 0; k <= pinwheelKMax; k++) {
    const wrapped =
      (((baseArg + (k * 2 * Math.PI) / 5) % ((2 * Math.PI) / 5)) + (2 * Math.PI) / 5) %
      ((2 * Math.PI) / 5);
  ...
  return {
    ...
    tables: [
      {
        title: 'Fundamental distance classes',
        columns: ['class', 'symbolic', '(r,s)', 'decimal', 'norm'],
        rows: classes.map((c) => [c.name, c.symbolic, `[${c.q5}]`, c.value, c.norm]),
      },
      {
        title: 'Ring catalog',
        columns: ['k', '(r,s)', 'decimal', '|R_k|', 'ratio', 'note'],
        rows: catalog.map((c) => [c.k, `[${c.q5}]`, c.value, c.mult, c.ratio, c.note]),
      },
      {
        title: 'Fibonacci spacing',
        columns: ['k', 'delta_k', 'ratio'],
        rows: spacing.map((s) => [s.k, s.value, s.ratio]),
      },
      {
        title: 'Norm representations',
        columns: ['N', 'count'],
        rows: normCounts.map((n) => [n.target, n.count]),
      },
      {
        title: 'D(n) scaling',
        columns: ['n', 'D_pent', 'D_Z2', 'ratio'],
        rows: bigComparison.map((r) => [r.n, r.D, r.z2, r.ratio]),
      },
    ],
    log: logger.text(),
  };
